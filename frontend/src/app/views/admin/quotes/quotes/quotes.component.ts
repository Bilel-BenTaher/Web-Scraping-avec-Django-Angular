...
saveQuote(): void {
    ...
    // VALIDATION : Seul le texte est obligatoire
    if (!this.editingQuote.text || this.editingQuote.text.trim() === '') {
    ...
    const quoteToSave = {
        ...this.editingQuote,
        text: this.editingQuote.text.trim(),
        author: this.editingQuote.author?.trim() || '', // Permettre vide
        tags: this.editingQuote.tags?.trim() || ''      // Permettre vide
    };
    
    this.isLoading = true;
    this.quoteService.updateQuote(quoteToSave.id, quoteToSave).subscribe({
        next: (updatedQuote) => {
  ...
  deleteQuote(quote: Quote): void {
    const authorName = quote.author || 'Unknown Author';
    if (confirm(`Êtes-vous sûr de vouloir supprimer la citation de ${authorName} ?`)) {
      this.isLoading = true;
